...
import { getPopulationRanking, type PopulationSort } from "@/lib/queries/populationAnalysis";
...
// que /grading-roi : filtre TCG + callout méthodologie + tableau trié, sauf
// pas de filtre "prix mini" (population n'est pas gated par un prix, elle
// existe indépendamment de tout prix connu). Chaque ligne ouvre le détail en
// modale (PopulationDetailModal), même convention que les autres pages
// d'analyse (cf. "Analyse en popup au clic").
// ─────────────────────────────────────────────────────────────────────────────
...
export default async function PopulationAnalysisPage({
  ...
  const { rows, totalCount } = await getPopulationRanking({ tcg, limit: PAGE_SIZE, page, sort });
